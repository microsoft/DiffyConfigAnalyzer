    ...
    internal class RegexParser
    {
        ...
        public Regex Parse()
        {
            ...
            if (symbol != string.Empty)
            {
                ...
                throw new Exception($"Unable to parse trailing: {remaining}");
            }

            return new RegexRemoveAnchorVisitor().Compute(ret);
        }
        ...
        /// </summary>
        /// <returns>A regex.</returns>
        private Regex ParseAtom()
        {
            if (AcceptNonSpecialCharacter(out var character))
            {
                return Regex.Char(character[0]);
            }
            else if (Accept("\\"))
            {
                if (Accept("e"))
            ...
            else if (Accept("["))
            {
                ...
                return isNegated ? Regex.Intersect(Regex.Negation(r), Regex.Dot()) : r;
            }
            else
            {
        ...
        /// </summary>
        /// <returns>A regex.</returns>
        private Regex ParseCharacterRange()
        {
            ...
            if (Accept("-"))
            {
                ...
                else
                {
                    ...
                    if (c2 < c1)
                    {
                        throw new Exception($"Invalid character range given.");
                    }
        ...
        private char ParseClassCharacter()
        {
            ...
            {
                throw new Exception($"Unexpected symbol: {symbol}, expected character or \\");
            }
        }
        ...
        private void Expect(string s)
        {
            if (!Accept(s))
            {
                throw new Exception($"Unexpected symbol {symbol}, expected: {s}");
            }
        }
        ...
        private string ExpectAnyCharacter()
        {
            if (symbol == string.Empty)
            {
                throw new Exception("Expected any character");
            }
